...
async function run() {
  ...
  try {
    ...
    for (const p of passengersData) {
      if (!familyMap[p.fam]) {
        const familyName = p.n.split(" ").slice(-1)[0] + " Family";
        const famId = "FAM" + Date.now() + Math.floor(Math.random()*1000);
        familyMap[p.fam] = famId;
      ...
      const passengerId = "P" + Date.now() + Math.floor(Math.random()*1000);
      ...
      let type = "Unknown";
      if (seatParts[1] === "L") type = "Lower";
      if (seatParts[1] === "M") type = "Middle";
      if (seatParts[1] === "U") type = "Upper";
      if (seatParts[1] === "SL") type = "Side Lower";
      if (seatParts[1] === "SU") type = "Side Upper";
